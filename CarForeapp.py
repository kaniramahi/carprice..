...
model = pickle.load(open('dt_modelcar.pkl','rb'))
df = pickle.load(open('datacar.pkl','rb'))
...
def main():
    ...
    sns.scatterplot(data=df, x="Age", y="selling_price", ax=ax) # corrected y-axis label
    st.pyplot(fig)

    # Add a pair plot of the numerical features
    st.subheader('Pair Plot of Numerical Features :bar_chart:')
    num_cols = ['Age', 'km_driven', 'fuel', 'seller_type', 'transmission', 'owner', 'selling_price','brand']
